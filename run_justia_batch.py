"""Fetch all Justia-backed states with fresh cache.

Clears old raw cache for each state before fetching so we get Justia data.
"""
import sys
import time
import json
...
def ingest_state(slug, clear_cache=True):
    from pipeline.ingestion.official_website import OfficialWebsiteIngestor

    # Clear old raw cache for this state so we re-fetch from Justia
    raw_cache = CACHE_DIR / "raw" / slug / "official"
    if clear_cache and raw_cache.exists():
        shutil.rmtree(raw_cache)
        print(f"  Cleared old cache for {slug}", flush=True)
    ...
    ingestor = OfficialWebsiteIngestor(state=slug, config=config, cache_dir=CACHE_DIR)
    start = time.time()
    try:
...
# States to fetch via Justia (all states that need data)
JUSTIA_STATES = [
    # Previously LexisNexis / 403-blocked
    "georgia", "mississippi", "tennessee", "arkansas",
    "new-jersey", "new-mexico", "maryland",
    # Official site failed (DNS/404/JS-only)
    "alabama", "texas", "indiana", "wyoming", "illinois",
    "utah", "new-york", "louisiana",
    # Low/no data from official sites
    "california", "colorado", "oklahoma", "south-dakota",
    "montana", "hawaii", "iowa", "kansas", "michigan",
    # Territories
...
if len(sys.argv) > 1:
    ...
    states = JUSTIA_STATES

print(f"Fetching {len(states)} states via Justia...", flush=True)
for i, slug in enumerate(states):
    print(f"  [{i+1}/{len(states)}] {slug}...", end=" ", flush=True)
